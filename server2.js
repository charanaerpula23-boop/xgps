...
app.post('/location', (req, res) => {
    ...
    if (latitude !== undefined && longitude !== undefined) {
        ...
        let previousLocation = latestLocations[deviceId];
        if (previousLocation) {
            const timeDiff = (ts - previousLocation.timestamp) / 1000; // seconds
        ...
        if (!locationHistory[deviceId]) locationHistory[deviceId] = [];
        ...
        latestLocations[deviceId] = newLocation;
        
        // Keep only the last 24 hours of history to avoid memory bloat
        const twentyFourHoursAgo = Date.now() - 24 * 60 * 60 * 1000;
        locationHistory[deviceId] = history.filter(loc => loc.timestamp >= twentyFourHoursAgo);
